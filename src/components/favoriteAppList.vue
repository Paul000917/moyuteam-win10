    ...
    <el-scrollbar style="height: 100%;" wrapStyle="overflow-x:hidden;">
        <transition name="favoriteapp-show">
            <div v-show="isShow" class="favoriteapp-panel">
                <el-row :gutter="10" :key="index" v-for="(cols,index) in appList">
                    <el-col :span="col.span" :key="key" v-for="(col,key) in cols">
                        <div v-if="col.hasOwnProperty('AppId')" class="bg-box" aniType="0" @click="openMenu(col.info)">
                            <div class="box-info">
                                <span>{{col.info}}</span>
                            </div>
                            <div class="box-img">
...
export default {
    ...
    created(){
        this.appList=testData.favoriteApp;
    },
    mounted(){
        ...
        this.$(".favoriteapp-panel .bg-box").hover(function(){
            ...
            if(type==1)
            {
                that.$(this).children('.box-img').css({'height':'80px','left':'0px','right':'auto','float':'left','opacity':'1'})
            }
            if(type==2)
            {
                that.$(this).children('.box-img').css({'height':'80px','bottom':'0px','top':'auto','float':'left','opacity':'1'})
            }
            if(type==3)
            {
    ...
    methods: {
        openMenu(info){
            this.$notify({title: info,message: '打开一个新窗口'});
